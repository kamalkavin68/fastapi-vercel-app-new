...
from api.nse.schema import EquityInfoSchema
...
class NseClient:
    ...
    def initialRequest(self):
        response = requests.request("GET", self.baseURL, headers={"User-Agent": self.userAgent})
        self.setCookies(response=response)
        print("Making Initial Request")

    def getEquityQuote(self, symbol: str):
        url = f"https://www.nseindia.com/api/quote-equity?symbol={symbol.upper()}"
        response = requests.request("GET", self.equityUrlParser(url), headers={"User-Agent": self.userAgent}, cookies=self.cookies, timeout=30)
        self.setCookies(response=response)
    ...
    def getFinalEquityQuote(self):
        ...
        try:
            listingData = datetime.strptime(stockMetaData.get("listingDate"), '%d-%b-%Y').replace(tzinfo=timezone_info)
        except Exception as error:
            listingData = None
        try:   
            lastUpdateTime = datetime.strptime(stockMetaData.get("lastUpdateTime"), '%d-%b-%Y %H:%M:%S')
        except Exception as error:
            lastUpdateTime = None
    ...
    def getEquityList(self) -> list[EquityInfoSchema]:
        ...
        for each in pd.read_csv(io.StringIO(response.text)).to_dict("records"):
            eachEquityData = EquityInfoSchema(symbol=each["SYMBOL"], nameOfCompany=each["NAME OF COMPANY"], series=each[" SERIES"],
                       dateOfListing=each[" DATE OF LISTING"], paidUpValue=each[" PAID UP VALUE"], marketLot=each[" MARKET LOT"],
                       isinNumber=each[" ISIN NUMBER"], faceValue=each[" FACE VALUE"])
